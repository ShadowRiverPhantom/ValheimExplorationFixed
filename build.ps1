<#
.SYNOPSIS
    Adapts the released Exploration plugin to the current Valheim build and packages it.

.DESCRIPTION
    Exploration 1.0.4 was built before Valheim 1.0 and no longer works on current builds. This
    script takes the upstream Exploration.dll and repairs it in two passes:

      1. src\fix-exploration.ps1    - restores the Harmony target that Valheim renamed, fixes the
                                      inverted treasure chest level test, and guards the
                                      Player.m_localPlayer dereferences on a dedicated server.
      2. src\fix-character-message.ps1 - retargets the Character.Message call sites to the
                                      five-parameter overload that Valheim 1.0 introduced.

    The result is validated, then written to dist\ together with a Thunderstore / r2modman
    package. Mono.Cecil from the BepInEx core is the only tooling needed - no C# compiler, no
    .NET SDK.

.EXAMPLE
    .\build.ps1
    .\build.ps1 -SourceDll "C:\Downloads\Exploration.dll" -Deploy
#>
[CmdletBinding()]
param(
    [string]$SourceDll,
...
function Find-UpstreamDll {
    ...
    $candidates = New-Object System.Collections.Generic.List[string]
    $fromEnv = [Environment]::GetEnvironmentVariable('EXPLORATION_SOURCE_DLL')
    ...
    throw "The upstream $Name.dll was not found. Download the Exploration 1.0.4 release, then either drop it in upstream\ or pass -SourceDll. See README.md."
}
...
# ---------------------------------------------------------------------------
# 1. Repair the plugin, then adapt it to Valheim 1.0
# ---------------------------------------------------------------------------

Write-Host ''
& (Join-Path $root 'src\fix-exploration.ps1') `
    -SrcDll $source `
    -OutDll $stagedDll `
...
Remove-Item -Path $stagedDll -Force -ErrorAction SilentlyContinue
